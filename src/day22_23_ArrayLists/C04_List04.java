...
public class C04_List04 {
	...
	public static void main(String[] args) {
		...
		List<Integer> sayilar = new ArrayList<>(); // yeni bir liste aktaracaðýz. onun için yeni list oluþturduk.
		
		
		for (int i = 0; i < arr.length; i++) {
			
			if (!sayilar.contains(arr[i])) {	// arr arraysýndeki elemanlarý içeriyor mu. içermiyorsa listeme eklemeliyim
				
				sayilar.add(arr[i]);		// burasý ekleme kýsmý.
		...
		Collections.sort(sayilar);		// burada sýraladýk.
